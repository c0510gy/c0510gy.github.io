...
실수는 무한 소수로 표현할 수 있으므로 $a_{nm} \in \{ n \in \mathbb{N} \vert 0 \le n \le 9 \}$이라 할 때 $f$를 다음과 같이 보일 수 있다.
...
여기서 어떤 집합의 멱집합이란, 어떤 집합의 모든 부분집합을 포함하는 집합을 말한다. 예를 들어, $X = \{ x, y, z \}$라고 하면 집합 $X$의 멱집합 $\mathcal{P}(X)$는 다음과 같다.
